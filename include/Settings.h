...
namespace ALYSLC
{
	struct Settings
	{
		...
		// and don't mind the jank of seeing the "void" outside the walkable exterior/interior terrain.
		...
		// Otherwise, one collision raycast for each ragdolling actor's main skeleton nodes each frame.
		static inline bool bSimpleActorCollisionRaycast = false;
		// WIP: Speed up supported dodge animations. Makes dodging more responsive but lowers dodge distance.
		...
		// revived by another player through health transfer or the pre-determined revive window elapses (entire party is wiped).
		...
		// Default input hold time threshold for triggering on-hold player actions, such as power attacks.
		...
		//[Progression]:
		//--------------
		// Add auto-scaling to co-op player's own skill progression when setting skill AVs on level up.
		...
		static inline std::vector<float> vfHealthRegenMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		static inline std::vector<float> vfMagickaCostMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		static inline std::vector<float> vfMagickaRegenMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		static inline std::vector<float> vfStaminaCostMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		static inline std::vector<float> vfStaminaRegenMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		// Ratio of health regen rate in combat to health regen rate out of combat.
		static inline std::vector<float> vfHealthRegenCombatRatio = { 0.70f, 0.70f, 0.70f, 0.70f };
		// Ratio of magicka regen rate in combat to magicka regen rate out of combat.
		static inline std::vector<float> vfMagickaRegenCombatRatio = { 0.33f, 0.33f, 0.33f, 0.33f };
		// Ratio of stamina regen rate in combat to stamina regen rate out of combat.
		static inline std::vector<float> vfStaminaRegenCombatRatio = { 0.35f, 0.35f, 0.35f, 0.35f };
		...
		static inline std::vector<float> vfDamageDealtMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		// Multiplier applied to each player's received damage.
		// Applies to most sources of damage, especially direct sources such as melee attacks or non-explosive hits.
		static inline std::vector<float> vfDamageReceivedMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		// Flop damage mult. For no reason at all.
		static inline std::vector<float> vfFlopDamageMult = { 1.0f, 1.0f, 1.0f, 1.0f };
		// Damage mult for thrown objects.
		static inline std::vector<float> vfThrownObjectDamageMult = { 1.0f, 1.0f, 1.0f, 1.0f };

		//-------------
		//[Controller]:
		//-------------
		// Proportion of full displacement from center (32768) to serve as analog stick deadzone.
		static inline std::vector<float> vfAnalogDeadzoneRatio = { 0.25f, 0.25f, 0.25f, 0.25f };
		// Proportion of full trigger activation (255) to serve as trigger deadzone.
		static inline std::vector<float> vfTriggerDeadzoneRatio = { 0.25f, 0.25f, 0.25f, 0.25f };

		//--------------
		//[Progression]:
		//--------------
		// Skill XP multiplier applied whenever this player receives XP for using a skill.
		static inline std::vector<float> vfSkillXPMult = { 1.0f, 1.0f, 1.0f, 1.0f };

		//------------
		//[Targeting]:
		//------------
		// Can this player select other players with their crosshair?
		static inline std::vector<bool> vbCanTargetOtherPlayers = { true, true, true, true };
		// Crosshair magnetism. Crosshair slows down when over a target.
		// Use crosshair magnetism when selecting Actors.
		static inline std::vector<bool> vbCrosshairMagnetismForActors = { true, true, true, true };
		...
		static inline std::vector<float> vfAimCorrectionFOV = { PI / 4.0f, PI / 4.0f, PI / 4.0f, PI / 4.0f };
		...
		static inline std::vector<bool> vbAnimatedCrosshair = { true, true, true, true };
		// Fade inactive crosshair.
		static inline std::vector<bool> vbFadeInactiveCrosshair = { true, true, true, true };
		// Recenter inactive crosshair.
		static inline std::vector<bool> vbRecenterInactiveCrosshair = { true, true, true, true };
		// [True]: Use Skyrim-style crosshair.
		// [False]: Use generic four-prongs style.
		static inline std::vector<bool> vbSkyrimStyleCrosshair = { true, true, true, true };
		// Crosshair gap radius (pixels).
		static inline std::vector<float> vfCrosshairGapRadius = { 3.0f, 3.0f, 3.0f, 3.0f };
		// Crosshair side length (pixels).
		static inline std::vector<float> vfCrosshairLength = { 7.0f, 7.0f, 7.0f, 7.0f };
		// Crosshair thickness (pixels).
		static inline std::vector<float> vfCrosshairThickness = { 4.0f, 4.0f, 4.0f, 4.0f };
		...
		static inline std::vector<float> vfCrosshairHorizontalSensitivity = { 0.5f, 0.5f, 0.5f, 0.5f };
		// Crosshair vertical movement sensitivities for each player.
		// Fraction of max pixels per second.
		// For example, a value of '0.5' means the crosshair will move
		// across a distance equal to half the max pixels traversable per second.
		static inline std::vector<float> vfCrosshairVerticalSensitivity = { 0.5f, 0.5f, 0.5f, 0.5f };
		// Player indicator side length (pixels).
		static inline std::vector<float> vfPlayerIndicatorLength = { 30.0f, 30.0f, 30.0f, 30.0f };
		// Player indicator side thickness (pixels).
		static inline std::vector<float> vfPlayerIndicatorThickness = { 5.0f, 5.0f, 5.0f, 5.0f };
		// Crosshair body is the same color as the overlay.
		// Red, Green, Cyan, Yellow by default.
		static inline std::vector<uint32_t> vuOverlayRGBAValues = { 0xFF0000AA, 0x00FF00AA, 0x00FFFFAA, 0xFF00FFAA };
		...
		// Seconds to rotate the crosshair from its default '+' orientation to its 'x' (face target) configuration or vice versa.
		static inline std::vector<float> vfSecsToRotateCrosshair = { 0.5f, 0.5f, 0.5f, 0.5f };
		...
		// Min calculated auto zoom delta at which to bail out of loop when attempting to keep all players in frame.
		...
		// Interpolation interval when calculating the average of player movement yaw angles relative to the camera's yaw.
		...
		// Max number of seconds to block all input actions after the player action manager resumes.
		...
		// Minimum distance (in-game units) to move before automatically refreshing nearby references when activation cycling.
		static inline const float fMinMoveDistToRefreshRefrs = 10.0f;
		// Minimum angle to turn before automatically refreshing nearby references when activation cycling.
		...
		// Max number of seconds over which the last released refr is handled following a collision.
		...
		static inline const std::filesystem::path defaultFilePathSSE = "Data/MCM/Config/ALYSLC/settings.ini";
		static inline const std::filesystem::path defaultFilePathEnderal = "Data/MCM/Config/ALYSLC Enderal/settings.ini";
		static inline const std::filesystem::path userSettingsFilePathSSE = "Data/MCM/Settings/ALYSLC.ini";
		static inline const std::filesystem::path userSettingsFilePathEnderal = "Data/MCM/Settings/ALYSLC Enderal.ini";
		...
		static bool ReadBoolSetting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, bool& a_settingOut);

		// Read a boolean setting with the given key from the given section into the given index in the outparam list.
		// Returns true if successful.
		static bool ReadBoolSettingToIndex(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, std::vector<bool>& a_settingsList, uint8_t&& a_index);
		...
		// Returns true if successful.
		static bool ReadFloatSetting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, float& a_settingOut, float&& a_lowerLimit, float&& a_upperLimit, bool&& a_convertToRadians = false);
		...
		// Returns true if successful.
		static bool ReadInt32Setting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, int32_t& a_settingOut, int32_t&& a_lowerLimit, int32_t&& a_upperLimit);
		...
		// Returns true if successful.
		static bool ReadRGBStringSetting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, uint32_t& a_settingOut);

		// Read a string setting with the given key from the given section into the outparam.
		// Returns true if successful.
		static bool ReadStringSetting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, RE::BSFixedString& a_settingOut);
