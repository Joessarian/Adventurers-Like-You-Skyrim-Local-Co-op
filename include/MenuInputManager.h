...
namespace ALYSLC 
{
	...
	struct MenuBindInfo
	{
		...
		// 0 when not pressed, 1 for fully pressed, > 0  and <= 1 for triggers.
		float value;
	...
	// Holds informatino that links player actions to menu opening requests.
	...
	struct MenuOpeningActionRequestsManager
	{
		...
		// Queues are traversed and cleared by default.
	...
	public:
		...
		// Draw outline around screen adges to indicate what player is controlling menus.
		// Overlay color matches the player's crosshair color.
		void DrawPlayerMenuControlOverlay();
		// NOTE: Called from hooks.
		...
		MenuInputEventType currentMenuInputType;
		...
		// Set and stack of co-op player-opened menu names.
		// Set contains hashes of menu names.
	...
	private:
		...
		// Level up co-op companion's skill by consuming the given Enderal skillbook.
		...
		// This data is used to create emulated input events to send.
		...
		// Done for quick slot items which are consumed by co-op companions,
		...
		// Input devices.
		const RE::INPUT_DEVICE gamepadDevice;
		const RE::INPUT_DEVICE keyboardDevice;
		const RE::INPUT_DEVICE mouseDevice;
		const RE::INPUT_DEVICE controllerDevice;

		// Menu contexts.
		const RE::UserEvents::INPUT_CONTEXT_ID bookContext;
		const RE::UserEvents::INPUT_CONTEXT_ID consoleContext;
		const RE::UserEvents::INPUT_CONTEXT_ID cursorContext;
		const RE::UserEvents::INPUT_CONTEXT_ID debugOverlayContext;
		const RE::UserEvents::INPUT_CONTEXT_ID favoritesContext;
		const RE::UserEvents::INPUT_CONTEXT_ID gameplayContext;
		const RE::UserEvents::INPUT_CONTEXT_ID itemMenuContext;
		const RE::UserEvents::INPUT_CONTEXT_ID inventoryContext;
		const RE::UserEvents::INPUT_CONTEXT_ID journalContext;
		const RE::UserEvents::INPUT_CONTEXT_ID lockpickingContext;
		const RE::UserEvents::INPUT_CONTEXT_ID mapContext;
		const RE::UserEvents::INPUT_CONTEXT_ID menuContext;
		const RE::UserEvents::INPUT_CONTEXT_ID statsContext;

		// Length of quick slot item/spell prefix tag ('[*QSS*] ' or '[*QSI*] ').
		// Used when setting new prefix to denote favorited item/spell as equipped in the quick slot.
		...
		// Set of Magic Menu entry indices for forms that are equipped by the menu-controlling player.
		std::set<uint32_t> magFormEquippedIndices;
		...
		// NOTE: If a magic form is not found within P1's known spells/shouts lists, 
		...
		// Time point at which last equip state delayed refresh request was made.
		...
		// Has the either placeholder spell been changed?
