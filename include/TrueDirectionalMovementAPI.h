...
namespace TDM_API
{
	...
	enum class APIResult : uint8_t
	{
		...
		// True Directional Movement currently must keep control of this resource 
	...
	public:
		...
		[[nodiscard]] virtual APIResult RequestDisableDirectionalMovement
		(
			PluginHandle a_myPluginHandle
		) noexcept = 0;
		...
		/// <param name="a_myPluginHandle">Your assigned plugin handle</param>
		/// <returns>OK, MustKeep, AlreadyGiven, AlreadyTaken</returns>
		[[nodiscard]] virtual APIResult RequestDisableHeadtracking
		(
			PluginHandle a_myPluginHandle
		) noexcept = 0;
		...
		virtual APIResult ReleaseDisableDirectionalMovement
		(
			PluginHandle a_myPluginHandle
		) noexcept = 0;
	...
	public:
		...
		/// If granted, you may use the SetPlayerYaw function and TDM will not adjust the yaw
		/// for the duration of your control.
		/// </summary>
		/// <param name="a_myPluginHandle">Your assigned plugin handle</param>
		/// <param name="a_yawRotationSpeedMultiplier">The speed multiplier to use 
		/// when smoothly rotating to the desired yaw. 0 is instant.</param>
		/// <returns>OK, MustKeep, AlreadyGiven, AlreadyTaken</returns>
		[[nodiscard]] virtual APIResult RequestYawControl
		(
			PluginHandle a_myPluginHandle, float a_yawRotationSpeedMultiplier
		) noexcept = 0;
		...
		/// <returns>OK, NotOwner</returns>
		virtual APIResult SetPlayerYaw
		(
			PluginHandle a_myPluginHandle, float a_desiredYaw
		) noexcept = 0;
	...
	public:
		...
		[[nodiscard]] virtual DirectionalMovementMode GetDirectionalMovementMode
		() const noexcept = 0;
	...
	public:
		/// <summary>
		/// Check if the target position for the camera is behind the target 
		/// when target lock is enabled.
		/// </summary>
		/// <returns>true if target lock is enabled AND the target position is behind the camera.
	...
	public:
		...
		/// <param name="a_myPluginHandle">Your assigned plugin handle</param>
		/// <returns>OK, MustKeep, AlreadyGiven, AlreadyTaken</returns>
		[[nodiscard]] virtual APIResult RequestDisableTargetLock
		(
			PluginHandle a_myPluginHandle
		) noexcept = 0;
	...
	(
		const InterfaceVersion a_interfaceVersion = InterfaceVersion::V5
	)
	{
