...
namespace ALYSLC
{
	...
	{
		// Generate the default player action params list 
		// and assign as the default for each player until binds are imported later.
		...
		for (auto i = !InputAction::kFirstAction; i <= !InputAction::kLastAction; ++i) 
		{
			...
			defPAParamsList[playerActionIndex] = PlayerActionParams
			(
				inputsList.empty() ? 
				PerfType::kDisabled : 
				ACTION_BASE_PERF_TYPES[playerActionIndex],
				ACTION_BASE_TRIGGER_FLAGS[playerActionIndex],
				inputsList,
	...
	) const noexcept
	{
		...
		{
			// Input action is a player action, not an input, so return a mask of 0.
	...
	std::vector<InputAction> PlayerActionInfoHolder::GetComposingInputs
	(
	...
	) const noexcept
	{
		...
		(
			a_paCompInputActionsLists[!a_playerAction - !InputAction::kFirstAction]
		);
		...
		// Decompose each input action into inputs.
		for (auto i = 0; i < composingInputActions.size(); ++i)
		{
					 ...
					 inputAction <= InputAction::kLastAction)
			{
				...
				if (!actionInputs.empty() && 
					a_recursionDepthOut < Settings::uMaxComposingInputsCheckRecursionDepth)
				{
					composingInputs.insert
					(
						composingInputs.end(), actionInputs.begin(), actionInputs.end()
					);
				}
			}
		}
