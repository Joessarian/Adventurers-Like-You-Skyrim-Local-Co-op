...
namespace ALYSLC
{
	void Settings::ImportAllSettings()
	{
		...
		{
			RE::DebugMessageBox(fmt::format("[ALYSLC] Invalid binds found:\nDefault settings.ini: {}\n{}: {}\nTroubleshooting info messages are in the ALYSLC.log file and have the [BINDS] tag.\nAll invalid binds were disabled.", 
				!defBindsSucc ? "[Invalid]" : "[Valid]", 
				ALYSLC::EnderalCompat::g_enderalSSEInstalled ? "ALYSLC Enderal.ini" : "ALYSLC.ini",
				!customBindsSucc ? "[Invalid]" : "[Valid]").c_str());
		}
	...
	bool Settings::ImportBinds(CSimpleIniA& a_ini)
	{
		...
		std::array<std::vector<InputAction>, (size_t)(InputAction::kActionTotal)> tempComposingActionsLists;
		...
		// Must quicklytap the bind twice to trigger the action.
		bool consecTap = false;
		// This action's perform type can be changed to 'OnConsecTap'.
		bool canChangeConsecTap = false;
		...
		{
			// NOTE: These structures will already be populated by the default data assigned in the PA Info Holder constructor.
			...
			for (auto actionIndex = !InputAction::kFirstAction; actionIndex <= !InputAction::kLastAction; ++actionIndex) 
			{
				...
				anyInputOrder = playerPAParams[bindIndex].triggerFlags.all(TriggerFlag::kDoNotUseCompActionsOrdering);
				consecTap = playerPAParams[bindIndex].perfType == PerfType::kOnConsecTap;
				...
				if (defPerfType == PerfType::kOnRelease || defPerfType == PerfType::kOnPress || defPerfType == PerfType::kOnConsecTap) 
				{
					canChangeConsecTap = true;
					ReadBoolSetting(a_ini, sectionName.data(), fmt::format("bAction{}ConsecTap", actionName).c_str(), consecTap);
				}

				ReadBoolSetting(a_ini, sectionName.data(), fmt::format("bAction{}MinHoldTime", actionName).c_str(), minHoldTime);

				// Can't have both min hold time and consecutive tap flags set, 
				// since their trigger conditions oppose one another.
				...
				else
				{
					...
					if (anyInputOrder)
					{
						playerPAParams[bindIndex].triggerFlags.set(TriggerFlag::kDoNotUseCompActionsOrdering);
					}
					else
					{
						playerPAParams[bindIndex].triggerFlags.reset(TriggerFlag::kDoNotUseCompActionsOrdering);
					}

					if (consecTap)
					{
						// Set new perform type.
						playerPAParams[bindIndex].perfType = PerfType::kOnConsecTap;
					}
					else if (defPerfType == PerfType::kOnConsecTap)
					{
						// If the default bind was set to consec tap, but the custom one unsets this flag,
						// set the bind perform type as 'OnRelease'.
						// Otherwise, do not change the perf type for this bind, 
						// which will use the default bind's perform type.
						playerPAParams[bindIndex].perfType = PerfType::kOnRelease;
					}

					if (minHoldTime)
					{
						// Actions with a minimum hold time must also ignore conflicting actions while the bind is held.
						// Otherwise, they won't trigger.
						playerPAParams[bindIndex].triggerFlags.set(TriggerFlag::kMinHoldTime, TriggerFlag::kIgnoreConflictingActions);

						// Switch perform type to 'OnPress' so that the action triggers right after the bind is held for longer than the hold threshold,
						// instead of on release of the bind or after tapping twice.
						if (defPerfType == PerfType::kOnConsecTap || defPerfType == PerfType::kOnRelease) 
						{
							playerPAParams[bindIndex].perfType = PerfType::kOnPress;
						}
					}
					else
					{
						// Remove hold time flag and keep default perform type.
						playerPAParams[bindIndex].triggerFlags.reset(TriggerFlag::kMinHoldTime);
						// If the default bind did not have a minimum hold time and did not already ignore conflicting actions, reset the flag here.
						if (playerPAParams[bindIndex].triggerFlags.none(TriggerFlag::kMinHoldTime, TriggerFlag::kIgnoreConflictingActions)) 
						{
							playerPAParams[bindIndex].triggerFlags.reset(TriggerFlag::kIgnoreConflictingActions);
						}
					}
				}
				...
				do
				{
					// Check if the composing action slot and the assigned input action index were marked as modified.
					// Get the composing action's enabled state if it exists.
					bool slotModified = ReadBoolSetting(a_ini, sectionName.data(), fmt::format("bAction{}{}", actionName, compActionSlot).c_str(), composingActionEnabled);
					// Get the composing action's assigned input action index if it exists.s
					bool slotInputActionIndexModified = ReadUInt32Setting(a_ini, sectionName.data(), fmt::format("uAction{}{}", actionName, compActionSlot).c_str(), assignedInputActionIndex, !InputAction::kNone, !InputAction::kTotal);
					if (slotModified && slotInputActionIndexModified) 
					{
						// Slot was toggled on/off by the player and its assigned InputAction was also modified.
						if (composingActionEnabled)
						{
							// Not zero-indexed when read in from .ini, since players indicate no assigned action with 0 (None).
							// Subtract 1 to get enum index.
							--assignedInputActionIndex;
							// No composing action assigned if outside the valid InputAction range.
							compAction = assignedInputActionIndex >= !InputAction::kFirst && assignedInputActionIndex <= !InputAction::kLastAction ? static_cast<InputAction>(assignedInputActionIndex) : InputAction::kNone;
							// If the composing action was already assigned before, skip it.
							alreadyAssigned = compAction != InputAction::kNone && !composingActions.empty() && composingActions.contains(compAction);

							if (assignedInputActionIndex != !InputAction::kNone && assignedInputActionIndex <= !InputAction::kLastAction && !alreadyAssigned)
							{
							...
							else if (assignedInputActionIndex == !InputAction::kNone || alreadyAssigned)
							{
								// Not assigned an action or already assigned previously. 
								// Nothing to add to the composing actions list or set.
							}
							else
							{
						...
						{
							// Slot skipped since it's disabled.
						}
					}
					else if (slotModified && !slotInputActionIndexModified)
					{
						// Slot was toggled on/off by the player but the assigned InputAction was not modified.
						if (composingActionEnabled) 
						{
							// Shift down to index into default composing InputActions list.
							if (compActionSlot - 1 < defPACompActionsList[bindIndex].size())
							{
								// Not modified, so assign the default InputAction if the slot was assigned one and it wasn't already assigned.
								compAction = defPACompActionsList[bindIndex][compActionSlot - 1];
								alreadyAssigned = !composingActions.empty() && composingActions.contains(compAction);
					...
					{
						// Slot was not toggled on/off by the player, but the assigned InputAction was.
						
						// Was the default composing action in this slot defined?
						// If not, since the player changed a composing action in a slot that is not enabled,
						// the assigned composing action will not be enabled.
						// Shift down to index into default composing InputActions list.
						composingActionEnabled = compActionSlot - 1 < defPACompActionsList[bindIndex].size();
						// Not zero-indexed when read in from .ini, since players indicate no assigned action with 0 (None).
						// Subtract 1 to get enum index.
						--assignedInputActionIndex;
						// No composing action assigned if outside the valid InputAction range.
						...
						{
							// Not an error.
							// Slot's action skipped since the slot is disabled, despite being assigned an action.
						}
					}
					else
					{
						// Slot was not toggled on/off and its assigned InputAction was also not modified.
					...
					// Loop while the current slot's action is enabled and the current slot is not past the last slot.
					// NOTE: Discontinuous action assignment, 
					// e.g. Slot 1: enabled with valid action, Slot 2: disabled, Slot 3: enabled with valid action,
					// is not supported, as only Slot 1's InputAction will be assigned.
					// Any actions assigned in enabled slots after the first disabled slot are ignored.
				} while (composingActionEnabled && compActionSlot <= GlobalCoopData::MAX_ACTIONS_PER_BIND);
			...
			for (auto actionIndex = !InputAction::kFirstAction; actionIndex <= !InputAction::kLastAction; ++actionIndex)
			{
				...
				playerPAParams[bindIndex].composingInputs = paInfo->GetComposingInputs(action, tempComposingActionsLists, numComposingPlayerActions, recursionDepth);
				...
				playerPAParams[bindIndex].inputMask = paInfo->GetInputMask(playerPAParams[bindIndex].composingInputs);
				playerPAParams[bindIndex].composingPlayerActionsCount = numComposingPlayerActions;
			}
	...
	void Settings::ImportGeneralSettings(CSimpleIniA& a_ini)
	{
		...
		ReadBoolSetting(a_ini, "Menus", "bUninitializedDialogueWithClosestPlayer", bUninitializedDialogueWithClosestPlayer);
		ReadFloatSetting(a_ini, "Menus", "fAutoEndDialogueRadius", fAutoEndDialogueRadius, 100.0f, 2000.0f);
		// Movement
		...
		ReadFloatSetting(a_ini, "Movement", "fJumpBaseLSDirSpeed", fJumpBaseLSDirSpeed, 0.0f, 2000.0f);
		ReadFloatSetting(a_ini, "Movement", "fJumpingGravityMult", fJumpingGravityMult, 0.1f, 10.0f);
		ReadFloatSetting(a_ini, "Movement", "fJumpingMaxSlopeAngle", fJumpingMaxSlopeAngle, 0.0f, 90.0f, true);
		ReadFloatSetting(a_ini, "Movement", "fMeleeAttackMovMult", fMeleeAttackMovMult, 0.1f, 1.0f);
		ReadFloatSetting(a_ini, "Movement", "fMeleeAttackRotMult", fMeleeAttackRotMult, 0.1f, 1.0f);
		ReadFloatSetting(a_ini, "Movement", "fRangedAttackMovMult", fRangedAttackMovMult, 0.1f, 1.0f);
		...
		ReadBoolSetting(a_ini, "PlayerActions", "bCanKillmoveOtherPlayers", bCanKillmoveOtherPlayers);
		...
		ReadFloatSetting(a_ini, "PlayerActions", "fSecsCyclingInterval", fSecsCyclingInterval, 0.1f, 3.0f);
		...
		ReadBoolSetting(a_ini, "Progression", "bScaleLearningPointsWithNumPlayers", bScaleLearningPointsWithNumPlayers);
		ReadBoolSetting(a_ini, "Progression", "bScaleMemoryPointsWithNumPlayers", bScaleMemoryPointsWithNumPlayers);
		ReadBoolSetting(a_ini, "Progression", "bEveryoneGetsALootedEnderalSkillbook", bEveryoneGetsALootedEnderalSkillbook);
		ReadFloatSetting(a_ini, "Progression", "fAdditionalGoldPerPlayerMult", fAdditionalGoldPerPlayerMult, 0.0f, 10.0f);
		ReadFloatSetting(a_ini, "Progression", "fLevelUpXPThresholdMult", fLevelUpXPThresholdMult, 0.1f, 4.0f);
		// UI
		ReadFloatSetting(a_ini, "UI", "fCamLockOnIndicatorLength", fCamLockOnIndicatorLength, 1.0f, 200.0f);
		ReadFloatSetting(a_ini, "UI", "fCamLockOnIndicatorThickness", fCamLockOnIndicatorThickness, 1.0f, 20.0f);
	...
	void Settings::ImportPlayerSpecificSettings(CSimpleIniA& a_ini)
	{
		...
		for (auto pIndex = 0; pIndex < ALYSLC_MAX_PLAYER_COUNT; ++pIndex) 
		{
			...
			// Progression
			ReadFloatSetting(a_ini, sectionName.data(), "fSkillXPMult", (float&)vfSkillXPMult[pIndex], 0.0f, 10.0f);
			// Targeting
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bCanTargetOtherPlayers", vbCanTargetOtherPlayers, pIndex);
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bCrosshairMagnetismForActors", vbCrosshairMagnetismForActors, pIndex);
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bCrosshairMagnetismForObjRefs", vbCrosshairMagnetismForObjRefs, pIndex);
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bFriendlyFire", vbFriendlyFire, pIndex);
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bUseAimCorrection", vbUseAimCorrection, pIndex);
			ReadFloatSetting(a_ini, sectionName.data(), "fAimCorrectionFOV", (float&)vfAimCorrectionFOV[pIndex], 0.0f, 360.0f, true);
			ReadFloatSetting(a_ini, sectionName.data(), "fMaxAimPitchAdjustmentRate", (float&)vfMaxAimPitchAdjustmentRate[pIndex], 0.0f, 720.0f, true);
			ReadUInt32Setting(a_ini, sectionName.data(), "uProjectileTrajectoryType", (uint32_t&)vuProjectileTrajectoryType[pIndex], 0, 2);
			// UI
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bAnimatedCrosshair", vbAnimatedCrosshair, pIndex);
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bSkyrimStyleCrosshair", vbSkyrimStyleCrosshair, pIndex);
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bFadeInactiveCrosshair", vbFadeInactiveCrosshair, pIndex);
			ReadBoolSettingToIndex(a_ini, sectionName.data(), "bRecenterInactiveCrosshair", vbRecenterInactiveCrosshair, pIndex);
			ReadFloatSetting(a_ini, sectionName.data(), "fCrosshairGapRadius", (float&)vfCrosshairGapRadius[pIndex], 0.0f, 100.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fCrosshairLength", (float&)vfCrosshairLength[pIndex], 0.0f, 100.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fCrosshairThickness", (float&)vfCrosshairThickness[pIndex], 0.0f, 10.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fCrosshairHorizontalSensitivity", (float&)vfCrosshairHorizontalSensitivity[pIndex], 0.0f, 1.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fCrosshairVerticalSensitivity", (float&)vfCrosshairVerticalSensitivity[pIndex], 0.0f, 1.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fPlayerIndicatorLength", (float&)vfPlayerIndicatorLength[pIndex], 1.0f, 100.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fPlayerIndicatorThickness", (float&)vfPlayerIndicatorThickness[pIndex], 1.0f, 10.0f);
			// Default RGB values:
			// P1: Red, P2: Green, P3: Cyan, P4: Yellow.
			uint32_t rgb = pIndex == 0 ? 0xFF0000 : 
						   pIndex == 1 ? 0x00FF00 :
						   pIndex == 2 ? 0x00FFFF :
						   pIndex == 3 ? 0xFFFF00 :
						   0xFFFFFF;
			...
			ReadUInt32Setting(a_ini, sectionName.data(), "uCrosshairInnerOutlineAlpha", alpha, 0, 0xFF);
			...
			ReadUInt32Setting(a_ini, sectionName.data(), "uCrosshairOuterOutlineAlpha", alpha, 0, 0xFF);
			vuCrosshairOuterOutlineRGBAValues[pIndex] = (rgb << 8) | alpha;
			ReadUInt32Setting(a_ini, sectionName.data(), "uPlayerIndicatorVisibilityType", (uint32_t&)vuPlayerIndicatorVisibilityType[pIndex], 0, 2);
			ReadFloatSetting(a_ini, sectionName.data(), "fMinSecsCrosshairTargetTraversal", (float&)vfMinSecsCrosshairTargetTraversal[pIndex], 0.0f, 2.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fSecsBeforeRemovingInactiveCrosshair", (float&)vfSecsBeforeRemovingInactiveCrosshair[pIndex], 0.5f, 10.0f);
			ReadFloatSetting(a_ini, sectionName.data(), "fSecsToOscillateCrosshair", (float&)vfSecsToOscillateCrosshair[pIndex], 0.1f, 10.0f);
	...
	bool Settings::ReadBoolSettingToIndex(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, std::vector<bool>& a_settingsList, uint8_t&& a_index)
	{
		// Read a boolean setting with the given key from the given section into the given index in the outparam list.
		// Returns true if successful.
	...
	bool Settings::ReadFloatSetting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, float& a_settingOut, float&& a_lowerLimit, float&& a_upperLimit, bool&& a_convertToRadians)
	{
		// Read a float setting with the given key from the given section into the outparam.
		// Also clamp the result to the given lower and upper bounds, and/or convert the result to radians.
		// Returns true if successful.

		if (auto keyValidPtr = a_ini.GetValue(a_section, a_settingKey); keyValidPtr)
		{
			a_settingOut = std::clamp(static_cast<float>(a_ini.GetDoubleValue(a_section, a_settingKey)), a_lowerLimit, a_upperLimit);
	...
	bool Settings::ReadInt32Setting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, int32_t& a_settingOut, int32_t&& a_lowerLimit, int32_t&& a_upperLimit)
	{
		// Read a signed 32-bit integer setting with the given key from the given section into the outparam.
	...
	bool Settings::ReadRGBStringSetting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, uint32_t& a_settingOut)
	{
		// Read an unsigned 32-bit integer setting with the given key from the given section and parse an RGB value from it into the outparam.
		// Returns true if successful.
	...
	bool Settings::ReadStringSetting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, RE::BSFixedString& a_settingOut)
	{
	...
	bool Settings::ReadUInt32Setting(CSimpleIniA& a_ini, const char* a_section, const char* a_settingKey, uint32_t& a_settingOut, uint32_t&& a_lowerLimit, uint32_t&& a_upperLimit)
	{ 
		// Read an unsigned 32-bit integer setting with the given key from the given section into the outparam.
		// Also clamp the result to the given lower and upper bounds.
		// Returns true if successful.
